...
def clean_flight_log(source_file_name, flight_log):
    ...
    flight_log = flight_log[["Microsec", "Flight_Date", "SenseLong", "SenseLat", "CH4", "Source_Name"]]
...
def clean_flight_log2(source_file_name, flight_log):
    # source_file_name = in_file.split("\\")[-1]
    ...
    CH4_copy = flight_log.loc[: , "CH4"].copy()
    CH4_copy = CH4_copy.astype("int")
    flight_log["CH4"] = CH4_copy
    
    time_ms = flight_log.loc[:, "Timestamp(ms)"].copy()
    ...
    flight_log = flight_log.loc[:, ["Flight_Date", "SenseLong", "SenseLat", "CH4", "Source_Name"]].copy()

    return flight_log


def save_and_write_peaks(df, peaks_file, fname):
    peaks_i = find_ch4_peaks(df["CH4"])
    peaks = df.iloc[peaks_i]
    peaks_list = list(zip(peaks["SenseLong"], peaks["SenseLat"], peaks["CH4"]))
    with open(peaks_file, 'a') as outfile:
        for peak in peaks_list:
            outfile.write(f"{peak[0]},{peak[1]},{peak[2]},{fname}\n")